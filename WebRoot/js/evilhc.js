...
combox.prototype.init = function() {
	var inputbox = "<input name='combox_" + this.name + "' onkeyup='"+this.obj+".keyup()'  onchange='"+ this.obj + ".find()'   ";
...
combox.prototype.keyup = function() {
	//有东西输入时
	var inputbox = document.getElementById("combox_" + this.name).value;
	var vals = new Array();
	with(this.select){
		var j=0;
		for (i = 0; i < options.length; i++){
			if (toPinyinShengmu(options[i].text).indexOf(inputbox) == 0) {
				vals[j] = options[i].text;
				j++;
			}
		}
	};
...
combox.prototype.find = function() {
	/* 当搜索到输入框的值时,下拉框自动定位 */
	var inputbox = document.getElementById("combox_" + this.name);
	if(inputbox.value==""){
		return;
	}
	with (this.select) {
		var j=1;
		for (i = 0; i < options.length; i++){
			if (toPinyinShengmu(options[i].text).indexOf(inputbox.value) == 0) {
